...
This file contains monthly co2 data collected from several different inspection stations, whose locations is shown
in the figure below.
...
library(tidyverse)
library(patchwork)
monthly_data = read.csv("./data/monthly_flask_co2.csv")
monthly_data[1:10,]
```
...
As it shows above, each inspection station has NA values in its column. The Alert station has the highest missing percentage and missing count while the Mauna Loa Observatory has the lowest missing percentage and missing count. What's more, the type of missing data is Missing at Random(MAR). We can prove it use the data of Kermadec Island Station and New Zealand Station.

```{r}
monthly_data %>% select('Yr', 'ker') %>% 
